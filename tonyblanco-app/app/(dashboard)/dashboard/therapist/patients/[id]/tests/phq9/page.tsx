...
export default function TherapistPatientPhq9Page() {
    ...
    const fetchResult = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/tests/results/?test_code=phq-9${patientId ? `&patient_id=${encodeURIComponent(patientId)}` : ""}`,
          {
            headers: {
        ...
        const items: PHQ9Result[] = data?.results || data || [];
        if (items.length === 0) {
          setResult(null);
          setNotes("");
        } else {
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h1 className="text-xl font-semibold text-gray-900">PHQ-9 (paciente)</h1>
  ...
  const progressPercent =
    totalScore !== null && totalScore !== undefined
      ? Math.min(Math.round((totalScore / 27) * 100), 100)
      : 0;

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">PHQ-9 — Resultado del paciente</h1>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
            <span>Progreso del puntaje (0–27)</span>
        ...
        {suicidalFlag && (
          <div className="rounded-md border border-amber-200 bg-amber-50 p-4">
            <p className="text-sm text-amber-800">
              Respuesta positiva en el ítem 9. Requiere seguimiento clínico. Interpretar con calma y coordinar acción profesional según protocolo.
            </p>
          </div>
        )}
