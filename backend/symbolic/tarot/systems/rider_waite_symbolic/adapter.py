...
def build_payload(selected_cards: list[str], context: dict[str, Any]) -> SymbolicReadingPayload:
    ...
    payload_id = "swm-v3-mock-rider-waite-" + "-".join([c["id"] for c in payload_cards])

    return SymbolicReadingPayload(
        id=payload_id,
        summary="Lectura educativa (mock) — Rider–Waite (simbólico). Observacional, no clínica.",
...
register_adapter(
    TarotSystemAdapter(
        ...
        aliases={"rider_waite", "rider_waite_symbolic"},
