...
function topEntries(record, labels, limit = 3) {
    return Object.entries(record)
        .sort((a, b) => b[1] - a[1])
        .slice(0, limit)
        .map(([id, weight]) => {
        const meta = labels[id];
        const intensity = weight >= 0.45 ? 'predominante' : weight >= 0.3 ? 'presente' : 'secundario';
        return {
            id,
            label: meta.label,
            weight,
...
function buildDominantFocus(treeState) {
...
function buildLatentGaps(treeState) {
    return treeState.sefirot
...
function buildPathProcesses(treeState, analysis) {
...
function detectProcessArc(paths, dominants) {
    if (paths.length === 0) {
    ...
    const hasHodYesodMalchut = ids.includes('hod') && ids.includes('yesod') && ids.includes('malchut');
...
function buildPolarityReading(analysis) {
    ...
    parts.push(`Armónica ${pct(harmonic)}, integrativa ${pct(integrative)}, tensional ${pct(tensional)}.`);
    return parts.join(' ');
}
function buildMethodBridge(context, dominants) {
    if (!context?.primaryNumbers?.length)
        return [];
    const lines = [];
    ...
    for (const num of context.primaryNumbers) {
        ...
        lines.push(`${num.label} = ${num.value}${titulo}${cualidad} → ${sefiraLabel} (peso ${num.weight.toFixed(2)}).`);
    }
    if (context.inclusionDominants?.length) {
        lines.push(`Casas dominantes en inclusión: ${context.inclusionDominants.join(', ')} — números con mayor repetición en el perfil simbólico.`);
    }
    if (context.inclusionAbsences?.length) {
        lines.push(`Casas ausentes: ${context.inclusionAbsences.join(', ')} — áreas de menor expresión numérica (exploración, no déficit).`);
    }
    const top = dominants[0];
    if (top) {
        const linked = context.primaryNumbers.find((n) => NUMBER_TO_SEFIRAH[n.value] === top.id);
        if (linked) {
            lines.push(`Coherencia método–árbol: el número ${linked.value} (${linked.label}) alimenta el foco en ${top.displayName}.`);
        }
    }
    return lines;
}
function buildSessionQuestions(dominants, paths, pillarAxes) {
    const questions = [];
    const top = dominants[0];
    const second = dominants[1];
    if (top) {
        questions.push(`¿Dónde la persona vive hoy la cualidad de ${top.displayName} (${top.light.toLowerCase()}) sin convertirla en performance?`);
        questions.push(`¿Qué versión sombría de ${top.displayName} podría estar activa (${top.shadowWatch.toLowerCase()})?`);
    }
    if (second && top) {
        questions.push(`¿Cómo se relacionan en sesión ${top.displayName} y ${second.displayName}? ¿Hay continuidad o ruptura entre ambas?`);
    }
    for (const path of paths.slice(0, 2)) {
        questions.push(`En el sendero ${path.fromLabel} → ${path.toLabel} (${path.narrative.toLowerCase()}): ¿qué está en tránsito y qué aún no baja de nivel?`);
    }
    const mainPillar = pillarAxes[0];
    if (mainPillar && mainPillar.weight >= 0.4) {
        questions.push(`El eje ${mainPillar.label} está cargado: ¿cómo se nota eso en la narrativa del consultante (${mainPillar.therapeuticAngle.toLowerCase()})?`);
    }
    if (questions.length < 5 && dominants.some((d) => d.id === 'malchut')) {
        questions.push('¿Qué gesto concreto, por pequeño, podría anclar el insight de la sesión en la vida diaria (Malkut)?');
    }
...
function buildHeadline(dominants, pillarAxes) {
    if (!dominants.length) {
...
function buildStructuralFocus(dominants, pillarAxes, analysis) {
    if (!dominants.length) {
...
function buildWorkingHypothesis(dominants, processArc, polarityReading, pillarAxes) {
    if (!dominants.length) {
...
function buildInterventionAngles(dominants, paths, latentGaps) {
    const angles = [];
    const top = dominants[0];
    if (top) {
        angles.push(`Trabajar la polaridad luz/sombra de ${top.displayName}: invitar ejemplos concretos donde ${top.tikkun.toLowerCase()}.`);
    }
    for (const path of paths.slice(0, 2)) {
        angles.push(`Seguir el sendero ${path.fromLabel} → ${path.toLabel}: nombrar qué ya transitó y qué queda «en el aire» antes de ${path.toLabel.split(' (')[0]}.`);
    }
    if (latentGaps[0]) {
        angles.push(`Explorar suavemente ${latentGaps[0].displayName} como zona poco habitada — sin interpretar ausencia como déficit.`);
    }
    if (dominants.some((d) => d.id === 'hod' || d.id === 'yesod')) {
        angles.push('Si hay mucha elaboración verbal, anclar con sensación corporal o imagen antes de más análisis (Hod/Yesod).');
    }
    if (dominants.some((d) => d.id === 'malchut') || paths.some((p) => p.to === 'malchut')) {
        angles.push('Cerrar con micro-compromiso observable (gesto, hábito, frontera) que materialice el insight (Malkut).');
    }
    return angles.slice(0, 5);
}
function buildTransferentialCues(dominants) {
    const cues = [];
    const top = dominants[0];
    if (!top) {
    ...
    if (top.id === 'hod' || byId.has('hod')) {
        cues.push('Riesgo de identificarse con la brillantez intelectual del consultante o competir en claridad conceptual.');
    }
    ...
    cues.push(`Tema ${top.displayName}: preguntarse si la respuesta propia hoy favorece ${top.tikkun.toLowerCase()} o refuerza la sombra.`);
    return cues.slice(0, 4);
}
function buildClinicalBridge(clinical, dominants, olamAxes) {
    if (!clinical)
        return [];
    const lines = [];
    if (clinical.ritmoState) {
        ...
        lines.push(`Ritmo álmico (${clinical.ritmoState}): ${ritmoNotes[clinical.ritmoState] ?? 'integrar con el relato del consultante.'}`);
    }
    if (clinical.mundoPredominante) {
        const treeOlam = olamAxes[0]?.label ?? '';
        lines.push(`Mundo clínico predominante: ${clinical.mundoPredominante}. Árbol marca ${treeOlam || 'distribución olamot'} — buscar convergencias, no forzar coincidencia.`);
    }
    if (clinical.harmonyIndex != null) {
        const level = clinical.harmonyIndex >= 0.7
    ...
    if (clinical.illuminatedSefirot?.length) {
        ...
        const overlap = clinical.illuminatedSefirot.filter((s) => treeTop.some((t) => t.toLowerCase().includes(s.toLowerCase()) || s.toLowerCase().includes(t.toLowerCase())));
        if (overlap.length) {
            lines.push(`Convergencia clínica–árbol en: ${overlap.join(', ')}.`);
        }
        else {
            lines.push(`Sefirot iluminadas en contexto clínico (${clinical.illuminatedSefirot.join(', ')}) difieren del ranking del método; explorar ambas lecturas.`);
        }
    }
    return lines;
}
function buildCoherenceNote(analysis) {
