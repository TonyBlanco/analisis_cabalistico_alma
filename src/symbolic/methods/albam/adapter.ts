import { calcularAnalisisAlbam } from './rules';
import { ALBAM_METADATA } from './tables';
import type { AlbamInput } from './types';

export function generarAnalisisAlbam(i: AlbamInput) {
	return calcularAnalisisAlbam(i) as any;
}

export function adaptarAEstadoSimbolicoAlbam(r: any): any {
	const pk = [
	...
	return { methodId: ALBAM_METADATA.id, methodName: ALBAM_METADATA.name, primaryNumbers: pk, inclusionMap: inclusion, rawData: r };
}

export function ejecutarMetodoAlbam(i: AlbamInput) {
	const r = generarAnalisisAlbam(i);
	return adaptarAEstadoSimbolicoAlbam(r);
}
