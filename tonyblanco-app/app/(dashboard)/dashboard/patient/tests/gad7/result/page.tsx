...
export default function Gad7PatientResultPage() {
  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${API_URL}/tests/results/?test_code=gad-7`, {
          headers: {
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h1 className="text-xl font-semibold text-gray-900">Mapa de Preocupación</h1>
  ...
  const progressPercent =
    totalScore !== null && totalScore !== undefined ? Math.min(Math.round((totalScore / 21) * 100), 100) : 0;

  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">Mapa de Preocupación — Resultado</h1>
        <p className="text-sm text-gray-600 mt-2">
          Lectura orientativa basada en 7 señales reportadas. No es diagnóstico.
        </p>
      </div>
