...
import { stressDefinition } from './stress.config';
...
export default function StressAssessmentPage() {
  ...
  const questions = stressDefinition.questions;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await executeTest({
        test_module_code: stressDefinition.code,
        input_data: {
      ...
      router.push('/dashboard/patient/tests/stress/result');
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{stressDefinition.name}</h1>
        <p className="text-sm text-gray-600 mt-2">
          {stressDefinition.purpose}. Tiempo estimado {stressDefinition.estimated_time_minutes} minutos.
        </p>
        <p className="text-xs text-gray-500 mt-2">
          Nota: este resultado es orientativo y no constituye un diagnóstico.
        </p>
        ...
        {questions.map((question, index) => (
          <div key={question.id} className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            <div className="text-xs text-gray-500 mb-2">
              Pregunta {index + 1} · Dimensión {question.dimension}
            </div>
            <h2 className="text-sm font-medium text-gray-900">{question.text}</h2>
            <div className="mt-4 space-y-2">
              {Object.entries(stressDefinition.scale.labels).map(([value, label]) => (
