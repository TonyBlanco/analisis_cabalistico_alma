...
import { unityIntrospectionDefinition } from './dudit-spirit.config';
import { executeTest } from '@/lib/test-api';

type AnswerMap = Record<string, string>;

export default function UnityIntrospectionPage() {
  const router = useRouter();
  const [answers, setAnswers] = useState<AnswerMap>({});
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const questions = unityIntrospectionDefinition.questions;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await executeTest({
        test_module_code: unityIntrospectionDefinition.code,
        input_data: {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{unityIntrospectionDefinition.name}</h1>
        <p className="text-sm text-gray-600 mt-2">
          {unityIntrospectionDefinition.purpose}. Tiempo estimado {unityIntrospectionDefinition.estimated_time_minutes} minutos.
        </p>
        <p className="text-xs text-gray-500 mt-2">Nota: cuestionario orientativo. No es diagnóstico.</p>
        ...
        {questions.map((question, index) => (
          <div key={question.id} className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-2">
              {Object.entries(unityIntrospectionDefinition.scale.labels).map(([value, label]) => (
