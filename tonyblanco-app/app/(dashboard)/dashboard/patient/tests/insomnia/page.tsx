...
import { insomniaDefinition } from './insomnia.config';
...
export default function InsomniaAssessmentPage() {
  const router = useRouter();
  const [answers, setAnswers] = useState<AnswerMap>({});
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const questions = insomniaDefinition.questions;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await executeTest({
        test_module_code: insomniaDefinition.code,
        input_data: {
      ...
      router.push('/dashboard/patient/tests/insomnia/result');
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{insomniaDefinition.name}</h1>
        <p className="text-sm text-gray-600 mt-2">{insomniaDefinition.purpose}. Tiempo estimado {insomniaDefinition.estimated_time_minutes} minutos.</p>
        <p className="text-xs text-gray-500 mt-2">Nota: cuestionario orientativo de bienestar. No es diagnóstico clínico.</p>
        ...
        {questions.map((question, index) => (
          ...
          >
            ...
            <div className="mt-4 space-y-2">
              {Object.entries(insomniaDefinition.scale.labels).map(([value, label]) => (
