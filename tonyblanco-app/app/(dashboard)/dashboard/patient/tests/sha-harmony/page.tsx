...
export default function SHAHarmonyPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await executeTest({
        test_module_code: shaHarmonyDefinition.code,
        input_data: {
      ...
      router.push('/dashboard/patient/tests/sha-harmony/result');
    } catch (e) {
      console.error('Error submitting SHA harmony test:', e);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{shaHarmonyDefinition.name}</h1>
        <p className="text-sm text-gray-600 mt-2">
          {shaHarmonyDefinition.purpose}. Tiempo estimado {shaHarmonyDefinition.estimated_time_minutes} minutos.
        </p>
        <p className="text-xs text-gray-500 mt-2">
          Nota: evaluación holística y simbólica de tu armonía interna. No es un diagnóstico médico.
        </p>
      ...
      {error && (
        <div className="bg-white border border-red-200 rounded-lg p-4">
          <p className="text-sm text-red-700 font-medium">Error en la consulta</p>
        ...
        {questions.map((question, index) => (
          <div key={question.id} className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            <div className="text-xs text-gray-500 mb-2">
              Pregunta {index + 1} — {question.dimension}
            </div>
            <h2 className="text-sm font-medium text-gray-900">{question.text}</h2>
            <div className="mt-4 space-y-2">
              {Object.entries(shaHarmonyDefinition.scale.labels).map(([value, label]) => (
