...
const WORKSPACE_SEED_EXERCISES = {
    'astrology-tarot': [
        {
            title: 'Carta del día',
            description: 'Elige una carta que represente cómo te sientes hoy y escribe, sin juzgar, qué imagen o recuerdo te evoca.',
            kind: 'symbolic-association',
        },
        {
            title: 'Diálogo con un eje',
            description: 'Escoge uno de los ejes que aparecieron en la sesión y describe qué te gustaría preguntarle si pudiera responderte.',
            kind: 'journaling',
        },
    ],
    'cabala-applied': [
        {
            ...
            description: 'Elige la sefirá con la que más resonaste y registra un momento del día en que sentiste su cualidad presente o ausente.',
            kind: 'reflection',
        },
        {
            title: 'Recorrido de un sendero',
            description: 'Visualiza con calma el sendero entre dos sefirot que trabajamos y observa qué sensaciones aparecen al recorrerlo.',
            kind: 'visualization',
        },
    ],
    transgenerational: [
        {
            ...
            description: 'Anota una frase que se repetía en tu familia y reflexiona, sin obligarte a concluir, sobre cómo la sientes hoy.',
            kind: 'journaling',
        },
        {
            title: 'Línea de los cuidados',
            description: 'Piensa en quién cuidó a quién en generaciones anteriores y observa qué emociones surgen al imaginar esa cadena.',
            kind: 'reflection',
        },
    ],
    generic: [
        {
            ...
            description: 'Elige un símbolo de la sesión de hoy y obsérvalo unos minutos; escribe después lo primero que aparezca, sin filtrarlo.',
            kind: 'reflection',
        },
        {
            title: 'Bitácora simbólica',
            description: 'Durante los próximos días, anota cualquier situación cotidiana que te recuerde a lo que exploramos en la sesión.',
            kind: 'journaling',
        },
    ],
};
 ...
 */
export function buildInteractiveExercises(state, options = {}) {
    ...
    for (const item of raw) {
        ...
        if (seen.has(key))
            continue;
        seen.add(key);
        const safety = validateSafetyContentForRole(`${item.title}\n${item.description}`, role);
        exercises.push({
