...
interface AstrologyVisualProRealProps {
...
export default function AstrologyVisualProReal({
  chart,
  analysisResult,
  activeLayers,
  onLayerToggle,
}: AstrologyVisualProRealProps) {
  ...
  const handleOverlayToggle = (key: 'natal' | 'transits' | 'solarReturn' | 'progressions') => {
    if (hasExternalLayers && onLayerToggle) {
      onLayerToggle(key);
      return;
    }
    setVisualState((prev) => ({
      ...
      <div className="flex items-start justify-between gap-3 border-b border-gray-200 px-4 py-3">
        <div>
          ...
          <p className="text-xs text-gray-600 mt-1">
            No se recalcula nada. Los controles filtran visibilidad; los datos permanecen identicos al resultado original.
          </p>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 p-4">
        ...
        <div className="rounded-lg border border-gray-200 p-3 space-y-3">
          <div>
            ...
            <p className="text-[11px] text-gray-500 mt-1">El orbe afecta solo a la visualizacion.</p>
          </div>
        ...
        <div className="rounded-lg border border-gray-200 p-3 space-y-3">
              ...
              filteredAspects.map((a) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 text-sm ${isActive ? 'bg-blue-50 border-l-2 border-blue-400' : 'bg-white'}`}
                  >
                    <div className="flex items-center justify-between">
          ...
          <div className="rounded-md border border-gray-100 bg-gray-50 px-3 py-2 text-xs text-gray-700">
            <p className="font-semibold text-gray-900 mb-1">Auditoria visual</p>
            <p>Tipo: Carta natal (visual)</p>
            <p>Sistema de casas: {meta.sistema_casas || '-'}</p>
            <p>Zodiaco: {meta.zodiac_type || '-'}</p>
            <p>Engine: {meta.fuente || 'kerykeion/swisseph'}</p>
            <p>Version engine: {meta.version_engine || '-'}</p>
