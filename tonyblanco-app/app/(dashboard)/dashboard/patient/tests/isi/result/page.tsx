...
export default function IsiPatientResultPage() {
  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${API_URL}/tests/results/?test_code=isi`, {
          headers: {
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h1 className="text-xl font-semibold text-gray-900">ISI</h1>
  ...
  const progressPercent =
    totalScore !== null && totalScore !== undefined ? Math.min(Math.round((totalScore / 28) * 100), 100) : 0;
