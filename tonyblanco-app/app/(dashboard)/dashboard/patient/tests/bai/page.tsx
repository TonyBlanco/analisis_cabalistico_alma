...
export default function BaiPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`${API_URL}/tests/bai/submit/`, {
        method: "POST",
        headers: {
      ...
      router.push("/dashboard/patient/tests/bai/result");
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{baiDefinition.name}</h1>
        <p className="text-sm text-gray-600 mt-2">
          {baiDefinition.purpose} para {baiDefinition.target_population}. Tiempo estimado {baiDefinition.estimated_time_minutes} minutos.
        </p>
