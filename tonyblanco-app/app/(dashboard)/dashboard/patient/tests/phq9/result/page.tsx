...
export default function Phq9PatientResultPage() {
  ...
  useEffect(() => {
    ...
    const fetchResult = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${API_URL}/tests/results/?test_code=phq-9`, {
          headers: {
  ...
  const totalScore = useMemo(() => {
    return (
      result?.result_data?.total_score ??
      result?.score ??
      null
    );
  }, [result]);
  ...
  const severity = useMemo(() => {
    return (
      result?.result_data?.severity_label ??
  ...
  const progressPercent = totalScore !== null && totalScore !== undefined ? Math.min(Math.round((totalScore / 27) * 100), 100) : 0;
