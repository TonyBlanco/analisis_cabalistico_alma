...
import { screeningGeneralDefinition } from './screening-general.config';
import { executeTest } from '@/lib/test-api';

type AnswerMap = Record<string, string>;

export default function ScreeningGeneralPage() {
  const router = useRouter();
  const [answers, setAnswers] = useState<AnswerMap>({});
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const questions = screeningGeneralDefinition.questions;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await executeTest({
        test_module_code: screeningGeneralDefinition.code,
        input_data: {
      ...
      router.push('/dashboard/patient/tests/screening-general/result');
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{screeningGeneralDefinition.name}</h1>
        ...
        {questions.map((question, index) => (
          <div key={question.id} className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-2">
              {Object.entries(screeningGeneralDefinition.scale.labels).map(([value, label]) => (
