...
import type { TreeStructuralState, SefiraId, FlowPolarity } from './tree-structural-state.types';
import { SEFIROT_TOPOLOGY, TREE_PATHS } from './tree-topology';
import type { PillarId, TriadId, OlamId } from './tree-topology';
import type { TreeStructuralAnalysis, GraphMetrics } from './tree-analysis.types';

// ─── Memoisation ────────────────────────────────────────────────────────────

const analysisCache = new Map<string, TreeStructuralAnalysis>();

function hashState(state: TreeStructuralState): string {
...
function computePillarBalance(
  state: TreeStructuralState,
): Record<PillarId, number> {
  const sums: Record<PillarId, number> = { severity: 0, mercy: 0, equilibrium: 0 };
  let total = 0;
  ...
  if (total === 0) return { severity: 0, mercy: 0, equilibrium: 0 };
...
function computeTriadActivation(
  state: TreeStructuralState,
): Record<TriadId | 'receptacle', number> {
  const sums: Record<TriadId | 'receptacle', number> = {
    supernal: 0, ethical: 0, astral: 0, receptacle: 0,
  };
  const counts: Record<TriadId | 'receptacle', number> = {
    supernal: 0, ethical: 0, astral: 0, receptacle: 0,
  };
...
function computeOlamActivation(
  state: TreeStructuralState,
): Record<OlamId, number> {
  const sums: Record<OlamId, number> = { atziluth: 0, beriah: 0, yetzirah: 0, assiah: 0 };
  const counts: Record<OlamId, number> = { atziluth: 0, beriah: 0, yetzirah: 0, assiah: 0 };
...
function computePolarityDistribution(
  state: TreeStructuralState,
): Record<FlowPolarity, number> {
  const counts: Record<FlowPolarity, number> = { harmonic: 0, integrative: 0, tensional: 0 };
  const total = state.flows.length;

  for (const f of state.flows) counts[f.polarity]++;

  if (total === 0) return { harmonic: 0, integrative: 0, tensional: 0 };
...
function computeGraphMetrics(state: TreeStructuralState): GraphMetrics {
  const activeNodeSet = new Set<SefiraId>(state.sefirot.map((s) => s.id));
  ...
  for (const flow of state.flows) {
    for (const path of TREE_PATHS) {
      if (
        (path.from === flow.from && path.to === flow.to) ||
  ...
  const degree: Partial<Record<SefiraId, number>> = {};
  ...
  const degreeCentrality: Record<SefiraId, number> = {
  ...
  for (const [id, val] of Object.entries(degree)) {
    degreeCentrality[id as SefiraId] = val as number;
  }

  // Build adjacency list from active paths (restricted to active nodes)
  const adj: Map<SefiraId, SefiraId[]> = new Map();
  for (const id of activeNodeSet) adj.set(id, []);
  for (const pathId of activePathIds) {
    const path = TREE_PATHS.find((p) => p.id === pathId);
    if (!path) continue;
  ...
  const visited = new Set<SefiraId>();
  let connectedComponents = 0;
  for (const startNode of activeNodeSet) {
    if (visited.has(startNode)) continue;
    connectedComponents++;
    const queue: SefiraId[] = [startNode];
  ...
  let longestActivePath: SefiraId[] = [];

  function dfs(node: SefiraId, visited: Set<SefiraId>, currentPath: SefiraId[]): void {
    if (currentPath.length > longestActivePath.length) {
      longestActivePath = [...currentPath];
    }
    if (currentPath.length >= 23) return; // hard cap: 22 edges = 23 nodes
    for (const neighbor of (adj.get(node) ?? [])) {
  ...
  for (const startNode of activeNodeSet) {
    const vis = new Set<SefiraId>([startNode]);
...
export function analyzeTreeState(state: TreeStructuralState): TreeStructuralAnalysis {
  const key = hashState(state);
  const cached = analysisCache.get(key);
  if (cached) return cached;

  const result: TreeStructuralAnalysis = {
    sourceVersion:       '0.2',
