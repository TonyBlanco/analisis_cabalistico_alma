 ...
 */
import { enforceAntiFraudRail, validateSafetyContentForRole, } from '../tree/clinical-lexicon';
...
function joinSections(summary, sections) {
 ...
 */
export function buildSessionNotesDraft(state, options = {}) {
    ...
    // Resumen
    const summary = state.summary && state.summary.trim().length > 0
        ? state.summary.trim()
