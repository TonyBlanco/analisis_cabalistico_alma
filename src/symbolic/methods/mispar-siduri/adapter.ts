import { calcularAnalisisMisparSiduri } from './rules';
import { MISPAR_SIDURI_METADATA } from './tables';
import type { MisparSiduriInput } from './types';

export function generarAnalisisMisparSiduri(i: MisparSiduriInput) {
	return calcularAnalisisMisparSiduri(i) as any;
}

export function adaptarAEstadoSimbolicoMisparSiduri(r: any): any {
	const pk = [
	...
	return { methodId: MISPAR_SIDURI_METADATA.id, methodName: MISPAR_SIDURI_METADATA.name, primaryNumbers: pk, inclusionMap: inclusion, rawData: r };
}

export function ejecutarMetodoMisparSiduri(i: MisparSiduriInput) {
	const r = generarAnalisisMisparSiduri(i);
	return adaptarAEstadoSimbolicoMisparSiduri(r);
}
