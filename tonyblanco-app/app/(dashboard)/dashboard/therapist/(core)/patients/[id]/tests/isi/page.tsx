...
export default function TherapistPatientIsiPage() {
  const params = useParams<{ id: string }>();
  const patientId = params?.id;

  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [result, setResult] = useState<ISIResult | null>(null);
    ...
    const fetchResult = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/tests/results/?test_code=isi${patientId ? `&patient_id=${encodeURIComponent(patientId)}` : ""}`,
          {
            headers: {
        ...
        const items: ISIResult[] = data?.results || data || [];
        if (items.length === 0) {
          setResult(null);
          setNotes("");
        } else {
  ...
  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <p className="text-sm text-gray-600">Cargando resultado ISI…</p>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h1 className="text-xl font-semibold text-gray-900">ISI (consultante)</h1>
  ...
  const progressPercent =
    totalScore !== null && totalScore !== undefined
      ? Math.min(Math.round((totalScore / 28) * 100), 100)
      : 0;

  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">Ritmo del Descanso — Resultado del consultante</h1>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
            <span>Progreso del puntaje (0–28)</span>
