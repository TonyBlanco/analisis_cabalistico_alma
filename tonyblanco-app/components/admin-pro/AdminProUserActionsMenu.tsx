...
export function AdminProUserActionsMenu(props: {
  ...
  const itemClass =
    'w-full px-3 py-2 text-left text-xs text-slate-900 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60';
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="inline-flex items-center rounded-md border border-slate-200 bg-white px-2 py-1 text-xs font-medium text-slate-900 hover:bg-slate-50 disabled:opacity-60"
        disabled={busy}
        aria-haspopup="menu"
        aria-expanded={open}
      >
