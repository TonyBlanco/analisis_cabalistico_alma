...
export default function YbocsSoulPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await executeTest({
        test_module_code: ybocsSoulDefinition.code,
        input_data: {
      ...
      router.push("/dashboard/patient/tests/ybocs-soul/result");
    } catch (e) {
  ...
  const renderQuestionGroup = (
    groupQuestions: typeof questions,
    groupLabel: string,
    globalOffset: number,
  ) => (
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{ybocsSoulDefinition.name}</h1>
        <p className="text-sm text-gray-600 mt-2">
          {ybocsSoulDefinition.purpose}. Tiempo estimado {ybocsSoulDefinition.estimated_time_minutes} minutos.
        </p>
        <p className="text-xs text-amber-600 mt-2 border border-amber-200 bg-amber-50 rounded px-3 py-2">
          {ybocsSoulDefinition.disclaimer}
        </p>
