...
export function aspectStyle(aspect: AspectLine): { stroke: string; width: number; opacity: number } {
  ...
  const base = (() => {
    switch (aspect.kind) {
      case "CONJ": return { stroke: "#8a6a00", w: 2.2 }; // conj (dorado sobrio)
      case "OPP":  return { stroke: "#b40000", w: 2.0 }; // rojo sobrio
      case "SQR":  return { stroke: "#b40000", w: 1.8 };
      case "TRI":  return { stroke: "#007a2f", w: 1.6 }; // verde sobrio
      case "SEXT": return { stroke: "#0b4aa6", w: 1.4 }; // azul sobrio
