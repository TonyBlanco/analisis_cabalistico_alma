...
export type AdminWorkspaceContractV2 = {
  meta: { version: 'admin-workspace.v2'; generated_at: string };
  system: {
...
function warnOnce(key: string, ...args: any[]) {
  ...
  warned.add(key);
  // NOTE: Required by spec: warn on unexpected backend shapes.
...
function statusFromResult(result: AdminProApiResult<unknown> | undefined): AdminSystemStatus {
  ...
  return result.status >= 500 ? 'degraded' : 'degraded';
}

export function buildAdminWorkspaceContractV2(args: {
  check?: AdminProApiResult<unknown>;
  stats?: AdminProApiResult<unknown>;
  users?: AdminProApiResult<unknown>;
  last_refresh_ms: number;
  capabilities: { can_patch_role: boolean; can_patch_active: boolean; can_delete: boolean };
}): AdminWorkspaceContractV2 {
  const generated_at = new Date().toISOString();

  const checks: AdminWorkspaceContractV2['system']['checks'] = [];
  ...
  const sections: AdminWorkspaceContractV2['ui']['sections'] = [
  ...
  return {
    meta: { version: 'admin-workspace.v2', generated_at },
