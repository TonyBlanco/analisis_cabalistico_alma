...
import { nutritionDefinition } from './nutrition.config';
...
export default function NutritionAssessmentPage() {
  ...
  const questions = nutritionDefinition.questions;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await executeTest({
        test_module_code: nutritionDefinition.code,
        input_data: {
      ...
      router.push('/dashboard/patient/tests/nutrition/result');
    } catch (e) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">{nutritionDefinition.name}</h1>
        <p className="text-sm text-gray-600 mt-2">
          {nutritionDefinition.purpose}. Tiempo estimado {nutritionDefinition.estimated_time_minutes} minutos.
        </p>
        ...
        {questions.map((question, index) => (
          <div key={question.id} className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-2">
              {Object.entries(nutritionDefinition.scale.labels).map(([value, label]) => (
