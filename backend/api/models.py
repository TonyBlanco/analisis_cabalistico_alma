...
class UserProfile(models.Model):
    ...
    # === Núcleo de perfil cabalístico/astrológico (PROFILE & CONSENT CORE) ===
    # Nombre legal completo (para consentimientos y documentos)
    legal_full_name = models.CharField(max_length=255, blank=True)

    # Datos de nacimiento canónicos del usuario (snapshot editable controlado)
    ...
    # Versión del perfil (para auditoría suave de cambios de datos base)
    profile_version = models.IntegerField(default=1)

    # Contador de cambios de nombre (espejo del sistema de UserBirthData)
    name_change_count = models.IntegerField(default=0)

    # Consentimiento terapéutico / tratamiento de datos aceptado
...
class AnalysisRecord(models.Model):
    """
    Núcleo normalizado de análisis.
    Representa UNA ejecución de análisis (clínico, cabalístico, astrológico o legacy).

    No reemplaza TestResult ni CabalisticAnalysis: convive con ellos.
    """
    ...
    # --- Birth data snapshot (INMUTABLE) ---
    birth_data_snapshot = models.JSONField(
        help_text=(
            "Snapshot inmutable de datos de nacimiento en el momento del análisis. "
            "Debe incluir: legal_name, birth_date, birth_time, city, country, "
            "lat, lng, timezone, geocode_source."
        ),
    )

    # --- Algorithm snapshot (INMUTABLE) ---
    ...
    # --- Anotaciones del terapeuta (editable, control de visibilidad) ---
    ...
    # Punteros opcionales a artefactos legacy para trazabilidad
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(fields=['kind', 'module_code']),
            models.Index(fields=['created_at']),
        ]

    def __str__(self):
        return f"{self.kind} / {self.module_code} @ {self.created_at.isoformat()}"

    def clean(self):
        """
        Validaciones defensivas a nivel de modelo:
        - Admin nunca es actor.
        - execution_mode solo para clinical_test.
        - therapist_clinical requiere patient + therapist + ownership + no auto-evaluación.
        """
        # Admin nunca es actor de análisis
        if self.created_by_user_id:
            profile = getattr(self.created_by_user, 'profile', None)
            if profile and (profile.is_admin or self.created_by_user.is_staff or self.created_by_user.is_superuser):
                raise ValidationError("Admin no puede ser actor de AnalysisRecord.")

        # execution_mode solo tiene sentido para clinical_test
        if self.execution_mode and self.kind != 'clinical_test':
            raise ValidationError("execution_mode solo es válido cuando kind='clinical_test'.")

        # Reglas específicas para therapist_clinical
        if self.execution_mode == 'therapist_clinical':
            if not self.patient_id or not self.therapist_id:
                raise ValidationError(
                    "therapist_clinical requiere patient y therapist asociados en AnalysisRecord."
                )

            # Ownership: el paciente debe pertenecer al terapeuta
            if self.patient.therapist_id != self.therapist_id:
                raise ValidationError(
                    "El paciente asociado no pertenece al terapeuta indicado en AnalysisRecord."
                )

            # No auto-evaluación: el terapeuta no puede ser su propio paciente
            if self.patient.user_id and self.patient.user_id == self.therapist_id:
                raise ValidationError("Auto-evaluación no permitida: el terapeuta no puede ser su propio paciente.")

    def save(self, *args, **kwargs):
        """
        Snapshots inmutables: birth_data_snapshot y algorithm_snapshot no pueden cambiar
...
class Resource(models.Model):
    """
    Recurso compartido (videos, audios, PDFs, cursos).
    Existe solo una vez; el acceso se gestiona vía UserResourceAccess.
    """
...
class UserResourceAccess(models.Model):
    """
    Relación de acceso usuario-recurso.
    Gestiona cómo un usuario obtiene acceso a un recurso.
    """
    SOURCE_CHOICES = [
        ('free', 'Gratuito'),
    ...
    def clean(self):
        """Validación: assigned_by solo permitido si source = assigned_by_therapist"""
        if self.source == 'assigned_by_therapist' and not self.assigned_by_id:
            raise ValidationError(
                "assigned_by es obligatorio cuando source='assigned_by_therapist'."
            )
        if self.source != 'assigned_by_therapist' and self.assigned_by_id:
            raise ValidationError(
                "assigned_by solo puede estar presente cuando source='assigned_by_therapist'."
            )
    ...
    def __str__(self):
        return f"{self.user.username} → {self.resource.title} ({self.get_source_display()})"
