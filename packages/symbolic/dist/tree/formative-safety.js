...
export function validateSafetyContent(content) {
    const warnings = [];
...
export const FORMATIVE_SAFE_DISCLAIMER = 'Lectura formativa y simbólica. No constituye evaluación clínica ni recomendación terapéutica automática. El terapeuta integra con su marco y el relato del consultante.';
const FIELD_FALLBACKS = {
    disclaimer: FORMATIVE_SAFE_DISCLAIMER,
    headline: 'Estructura simbólica activa; lectura exploratoria en marco formativo.',
    workingHypothesis: 'Hipótesis exploratoria: podría explorarse la organización simbólica sin cerrar sentido prematuramente.',
    structuralFocus: 'Foco estructural disponible; conviene integrar con el relato del consultante en supervisión.',
...
const GENERIC_FALLBACK = 'Observación estructural-simbólica (podría explorarse en sesión con marco formativo).';
export function fallbackForFormativeField(fieldPath) {
    const root = fieldPath.split('[')[0]?.split('.')[0] ?? fieldPath;
    return FIELD_FALLBACKS[root] ?? FIELD_FALLBACKS[fieldPath] ?? GENERIC_FALLBACK;
}
export function sanitizeFormativeField(fieldPath, text) {
...
export class FormativeBriefSafetyGateError extends Error {
    constructor(violations) {
        super(`Formative brief safety gate: ${violations.length} field(s) required sanitization: ${violations
            .map((v) => v.field)
            .join(', ')}`);
...
export function applyFormativeBriefSafetyGate(brief, options = {}) {
    const violations = [];
    const out = structuredClone(brief);
    const gate = (fieldPath, value) => {
    ...
    out.interventionAngles = out.interventionAngles.map((line, i) => gate(`interventionAngles[${i}]`, line));
    out.transferentialCues = out.transferentialCues.map((line, i) => gate(`transferentialCues[${i}]`, line));
    out.clinicalBridge = out.clinicalBridge.map((line, i) => gate(`clinicalBridge[${i}]`, line));
    out.sessionQuestions = out.sessionQuestions.map((line, i) => gate(`sessionQuestions[${i}]`, line));
    out.supervisionPrompts = out.supervisionPrompts.map((line, i) => gate(`supervisionPrompts[${i}]`, line));
...
export function collectFormativeBriefTextChunks(brief) {
    const chunks = [
        brief.headline,
