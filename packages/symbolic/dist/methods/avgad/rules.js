import { latinToHebrew, avgadTransform, calcGematriaStandard, reduceToSingleDigit, findKnownWords, numberToSefira, GEMATRIA_STANDARD } from '../../utils/hebrew-gematria';
/**
 * Calcula Avgad - Cada letra avanza una posición
 * א→ב, ב→ג, ת→א (circular)
 */
export function calcularAnalisisAvgad(input) {
    ...
    const transformedWords = hebrewWords.map((hw, idx) => {
        const transformed = hw.split('').map(avgadTransform).join('');
    ...
    return {
        ...
        calculo: {
            metodo: 'Avgad',
            descripcion: 'Cifrado por avance de una posición: א→ב, ב→ג, ת→א',
            explicacion: 'El Avgad desplaza cada letra una posición hacia adelante en el alfabeto, revelando significados potenciales ocultos.',
