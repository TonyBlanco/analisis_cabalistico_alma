...
function enrichFlow(f) {
    for (const path of TREE_PATHS) {
        if ((path.from === f.from && path.to === f.to) ||
...
function determineRole(activation) {
    if (activation >= 0.7)
        return 'dominant';
    if (activation >= 0.4)
        return 'present';
 ...
 */
function calculateFlowIntensity(fromWeight, toWeight) {
 ...
 */
function determineFlowPolarity(fromNum, toNum, fromWeight, toWeight) {
 ...
 */
function determineFlowDirection(fromNum, toNum) {
    // En el Árbol, números bajos están arriba (Keter=1) y altos abajo (Yesod=9)
    if (fromNum < toNum)
        return 'down'; // Descenso (de corona a fundamento)
    if (fromNum > toNum)
        return 'up'; // Ascenso (de fundamento a corona)
...
export function adaptPitagorasToTree(pitagorasState) {
    const sefirotMap = {}; // id → max activation
    ...
    pitagorasState.primaryNumbers.forEach((num) => {
        ...
        if (!sefiraId)
            return; // Skip si número no tiene mapeo directo
    ...
    for (const numStr in pitagorasState.inclusionMap) {
        if (!pitagorasState.inclusionMap.hasOwnProperty(numStr))
            continue;
