...
function getInsomniaComputed(result: TestResult | null): InsomniaComputed | null {
  const payload: any = result?.result_data;
  const computed = payload?.result ?? payload;
  return computed || null;
}

export default function InsomniaResultPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setError(null);
      try {
        const all = await getTestResults({ test_code: 'insomnia' });
  ...
  const computed = useMemo(() => getInsomniaComputed(result), [result]);
    ...
    return (
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          <h1 className="text-xl font-semibold text-gray-900">Insomnia — Descanso y hábitos</h1>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
        <h1 className="text-2xl font-semibold text-gray-900">Insomnia — Resultado</h1>
        <p className="text-sm text-gray-600 mt-2">Tu descanso: lectura orientativa de hábitos y sensaciones relacionadas con el sueño.</p>
      </div>

      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <p className="text-xs text-gray-500">Índice de sueño</p>
