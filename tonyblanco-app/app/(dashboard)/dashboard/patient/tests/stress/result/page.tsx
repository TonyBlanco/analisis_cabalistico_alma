...
function getStressComputed(result: TestResult | null): StressComputed | null {
...
export default function StressResultPage() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      setError(null);
      try {
        const all = await getTestResults({ test_code: 'stress' });
  ...
  const computed = useMemo(() => getStressComputed(result), [result]);
  const index = computed?.puntuaciones?.indice_0_100 ?? null;
  const level = computed?.puntuaciones?.nivel ?? null;
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          ...
          <div>
            ...
            <p className="text-lg font-medium text-gray-900">{level ?? 'N/A'}</p>
          </div>
        </div>
